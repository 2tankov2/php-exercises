...
/**
 * Адаптер – популярный шаблон проектирования. Он используется тогда, когда нужно использовать код, не
 * поддерживающий необходимый интерфейс. В такой ситуации, создается "обертка" над необходимым кодом,
 ...
 * Для генерации паролей, этот класс использует внешний объект, класс которого реализует интерфейс
 * PasswordGeneratorInterface.

 * Суть данного задания, внедрить в эту систему внешнюю библиотеку, которая не поддерживает интерфейс
 * PasswordGeneratorInterface.

 * Обратите внимание на то, что задача решается не через исправление кода самой библиотеки, а за счет
 * создания адаптера, благодаря которому соединяется код задания и код библиотеки.

src/HackzillaPasswordGeneratorAdapter.php
 * Напишите класс HackzillaPasswordGeneratorAdapter, который представляет собой адаптер к пакету
 * hackzilla/password-generator. Реализуйте внутри него интерфейс PasswordGeneratorInterface.
 ...
 * numbers
 * symbols
 * Каждая из этих опций, соответствует опциям внутри библиотеки hackzilla/password-generation. В
 * официальной документации видно как их можно установить. Значение по умолчанию для данных опций,
 * должно быть false.
 */

 // src/PasswordGeneratorInterface
...
// src/HackzillaPasswordGeneratorAdapter
